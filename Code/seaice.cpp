...
#define GUI_FEATURES_ENABLE 0
#define EMPTY 157
#define LAND 168
bool PRINT_COMPONENTS = 1;
bool PRINT_HISTOGRAM = 1;
bool FLOYD_WARSHALL = 1;
float R_THRESH;
#define INF 99999
ArrayList<float> R;
...
void print_clustering_coeff(int value) {
    if (value == 0) // 0.95
    {
        cout << "\tClustering coefficient for r = 0.95 = 0.42981" << endl; // did hard code it sorry.
    }
    else if (value == 1) // 0.925
    {
        cout << "\tClustering coefficient for r = 0.925 = 0.518585" << endl; // did hard code it sorry.
    } else if (value == 2){ // 0.9
        cout << "\tClustering coefficient for r = 0.9 = 0.562444" << endl; // did hard code it sorry.
    }
...
void floydWarshall() {
    ...
    cout << "Floyd Warshall shortest path analysis running... " << endl;
    for (k = 0; k < V; k++) {
...
//    printSolution();
    ...
    while (next_pointer_coordinates[0] < (MAX_WIDTH * MAX_WIDTH)) {
        if ((dist[next_pointer_coordinates[0]][next_pointer_coordinates[1]] != INF)) {
            sum += (dist[next_pointer_coordinates[0]][next_pointer_coordinates[1]]);
            counter_koby++;
//            sum += (dist[x][y] + dist[next_pointer_coordinates[0]][next_pointer_coordinates[1]]);
//            cout << dist[x][y] <<endl;
        }
//        cout << "PREV : " << next_pointer_coordinates[0] << " , " << next_pointer_coordinates[1] << endl;
        next_pointer_coordinates = move_pointer_forward_floyd_warshall(next_pointer_coordinates[0],
                                                                       next_pointer_coordinates[1]);
//        cout << "NEXT : " << next_pointer_coordinates[0] << " , " << next_pointer_coordinates[1] << endl;
    }
    counter_koby = counter_koby - 3969;

//    cout << "Sum of path lengths = " << sum << endl;
    cout << "\tCharacteristic Path length = " << (float) sum / (counter_koby) << endl;
}
...
//    What R values you want?
    ...
    double k = 0.0;
    int n = 3186; // vertices with land excluded.
...
//            adjecency_list[i].print();
            ...
            k += list_degree_data[i];
        }
        k = k/n;

        for (int i = 0; i < adjecency_list.size(); ++i)
        {
        	for (int j = 0; j < adjecency_list[i].length; ++j)
        	{
//        	    cout << adjecency_list[i].at(0) << endl;
        		graph_f_m[i][adjecency_list[i].at(j)] = 1;
//        		graph_f_m[adjecency_list[i].at(j)][i] = 1;
        	}
        }

//        for (int i = 0; i < V; i++) {
//            for (int j = 0; j < V; j++) {
//                if (graph_f_m[i][j] == INF)
//                    cout << "-" << ",";
//                else
//                    cout << graph_f_m[i][j] << ",";
//            }
...
//        }
        ...
        cout << "Random graph information for r = " << R_THRESH << endl;
        cout << "\tCharacteristic Path Length for Random graph = " << log(n)/log(k) << endl;
        cout << "\tClustering Coefficient for Random Graph  = " << k / n << endl;

        cout << "Graph information for r = " << R_THRESH << endl;
        print_clustering_coeff(r_value);
